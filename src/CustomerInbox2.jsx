...
const CustomerInbox2 = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API_BASE_URL}/artwork-feedback/delete/${id}`);
  ...
  const handleOpenChat = async (feedback) => {
    try {
      ...
      await axios.put(
        `${API_BASE_URL}/artwork-feedback/${feedback._id}/mark-read-customer`
      );
  ...
  const fetchFeedbackData = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/artwork-feedback?userId=${user._id}`
      );
  ...
  const handleSendReply = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/artwork-feedback/reply/${selectedFeedback._id}`,
        {
          message: newReply,
          sender: "customer",
        }
              ...
              >
                <Box
                  sx={{
                    ...
                    borderRadius: 2,
                    bgcolor: isSenderMessage(message) ? "#FF9F8F" : "#FFFFFF",
                    ...
                    "& fieldset": {
                      border:
                        selectedFeedback?.status === "resolved"
                          ...
                          : "1px solid #FF9F8F", // Default border when enabled
                    },
                    "&:hover fieldset": {
                      borderColor:
                        selectedFeedback?.status === "resolved"
                          ? "none" // Ensure no border on hover when disabled
                          : "#FF9F8F", // Darker blue on hover when enabled
                    },
                    "&.Mui-focused fieldset": {
                      borderColor:
                        selectedFeedback?.status === "resolved"
                          ? "none" // Ensure no border on focus when disabled
                          : "#FF9F8F", // Keep blue on focus when enabled
                    },
                  },
